...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Backend | SelfDevBuildTarget::Auto => vec![("iagent", "iagent")],
    };
...
mod tests {
    ...
    #[test]
    fn is_iagent_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_iagent_repo(repo.path()));
    }
}

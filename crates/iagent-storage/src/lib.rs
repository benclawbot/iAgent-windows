...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = iagent_core::fs::set_directory_permissions_owner_only(path);
    }
}
...
pub fn harden_user_config_permissions() {
    ...
    if let Ok(iagent_home) = iagent_dir()
        && iagent_home.exists()
    {
        let _ = iagent_core::fs::set_directory_permissions_owner_only(&iagent_home);
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    if let Some(parent) = path.parent() {
        let _ = iagent_core::fs::set_directory_permissions_owner_only(parent);
    }
    if path.exists() {
        let _ = iagent_core::fs::set_permissions_owner_only(path);
    }
}
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        iagent_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
pub fn write_text_secret(path: &Path, content: &str) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        iagent_core::fs::set_directory_permissions_owner_only(parent)?;
    }

...
mod tests {
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "iagent-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  iagent-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *iagent-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("iagent-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("iagent-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  iagent-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "iagent-linux-x86_64", bytes).unwrap();
    }

...
/// Mutate the process environment for iagent runtime configuration.
///
/// Rust 2024 makes environment mutation unsafe because it can race with
/// concurrent environment access in foreign code. iagent intentionally mutates
...
where
    ...
    V: AsRef<OsStr>,
{
    // SAFETY: iagent treats these mutations as process-global configuration.
...
/// Remove a process environment variable used by iagent runtime configuration.

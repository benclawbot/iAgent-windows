...
from right modifiers. iAgent's PRD § Question 5c explicitly tightens
...
def _normalize_key(key: keyboard.Key | keyboard.KeyCode | None) -> str | None:
    ...
    if isinstance(key, keyboard.Key):
        ...
        if key == keyboard.Key.ctrl_r:
            return "ctrl_r"
        # Some Windows/input-stack combinations report generic ctrl/alt
        # instead of side-specific variants. Treat them as the expected
        # modifiers so Ctrl+Alt binding remains reliable.

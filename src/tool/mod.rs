...
pub(crate) use iagent_tool_core::intent_schema_property;
pub use iagent_tool_core::{StdinInputRequest, Tool, ToolContext, ToolExecutionMode};
...
impl Registry {
    ...
    pub async fn register_mcp_tools(
        ...
        if server_count > 0 {
            log_info!(("MCP: Found {} server(s) in config", server_count));
            ...
            tokio::spawn(async move {
                ...
                if successes > 0 {
                    log_info!(("MCP: Connected to {} server(s)", successes));
                }
                if !failures.is_empty() {
                    for (name, error) in &failures {
                        log_error!(("MCP '{}' failed: {}", name, error));
                    }
                }

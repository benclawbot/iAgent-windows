    ...
    pub(super) async fn do_reload(
        ...
        if !target_binary.exists() {
            return Ok(ToolOutput::new(
                format!(
                    "No binary found at {}.\n\
                     Run 'iagent self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p iagent --bin iagent' and then try reload again.",
        ...
        if let Err(e) = reload_ctx.save() {
            log_error!(("Failed to save reload context: {}", e));
            let _ = build::rollback_pending_activation_for_session(session_id);
            return Err(e);
        }

...
fn auth_issue_runtime_display_name_tracks_direct_compatible_profiles() {
    ...
    crate::env::set_var("IAGENT_RUNTIME_PROVIDER", "azure-openai");
...
fn auth_profile_env_application_flushes_stale_openrouter_catalog_state() {
    ...
    crate::env::set_var("IAGENT_OPENROUTER_API_BASE", "https://openrouter.ai/api/v1");
    crate::env::set_var("IAGENT_OPENROUTER_API_KEY_NAME", "OPENROUTER_API_KEY");
    crate::env::set_var("IAGENT_OPENROUTER_ENV_FILE", "openrouter.env");
    crate::env::set_var("IAGENT_OPENROUTER_CACHE_NAMESPACE", "openrouter");
    crate::env::set_var("IAGENT_OPENROUTER_PROVIDER_FEATURES", "1");
    crate::env::set_var("IAGENT_OPENROUTER_ALLOW_NO_AUTH", "1");
    crate::env::set_var(
        "IAGENT_OPENROUTER_MODEL_CATALOG",
        "stale-openrouter-catalog.json",
    );
    crate::env::set_var("IAGENT_OPENROUTER_MODEL", "gpt-5.5");
    crate::env::set_var(
        "IAGENT_OPENROUTER_STATIC_MODELS",
        "stale-openrouter-only-model",
    );
    crate::env::set_var("IAGENT_OPENROUTER_AUTH_HEADER", "Bearer stale");
    crate::env::set_var("IAGENT_OPENROUTER_AUTH_HEADER_NAME", "Authorization");
    crate::env::set_var("IAGENT_OPENROUTER_DYNAMIC_BEARER_PROVIDER", "openrouter");
    crate::env::set_var("IAGENT_OPENROUTER_PROVIDER", "openrouter");
    crate::env::set_var("IAGENT_OPENROUTER_NO_FALLBACK", "1");
    crate::env::set_var("IAGENT_NAMED_PROVIDER_PROFILE", "openrouter");
    crate::env::set_var("IAGENT_PROVIDER_PROFILE_ACTIVE", "1");
    crate::env::set_var("IAGENT_PROVIDER_PROFILE_NAME", "openrouter");

    force_apply_openai_compatible_profile_env(Some(CEREBRAS_PROFILE));

    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_API_BASE").as_deref(),
        Ok("https://api.cerebras.ai/v1")
    );
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_API_KEY_NAME").as_deref(),
        Ok("CEREBRAS_API_KEY")
    );
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_ENV_FILE").as_deref(),
        Ok("cerebras.env")
    );
    assert_eq!(
    ...
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_PROVIDER_FEATURES").as_deref(),
        Ok("0")
    );
    assert!(std::env::var_os("IAGENT_OPENROUTER_ALLOW_NO_AUTH").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_MODEL_CATALOG").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_MODEL").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_AUTH_HEADER").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_AUTH_HEADER_NAME").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_DYNAMIC_BEARER_PROVIDER").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_PROVIDER").is_none());
    assert!(std::env::var_os("IAGENT_OPENROUTER_NO_FALLBACK").is_none());
    assert!(std::env::var_os("IAGENT_NAMED_PROVIDER_PROFILE").is_none());
    assert!(std::env::var_os("IAGENT_PROVIDER_PROFILE_ACTIVE").is_none());
    assert!(std::env::var_os("IAGENT_PROVIDER_PROFILE_NAME").is_none());
    assert_ne!(
        std::env::var("IAGENT_OPENROUTER_STATIC_MODELS")
            .ok()
            .as_deref(),
...
fn matrix_openrouter_like_sources_include_all_static_profiles() {
    ...
    crate::env::remove_var("IAGENT_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("IAGENT_OPENROUTER_ENV_FILE");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_ENV_FILE");
...
fn matrix_openrouter_like_sources_accept_valid_overrides() {
    ...
    crate::env::set_var("IAGENT_OPENROUTER_API_KEY_NAME", "ALT_OPENROUTER_KEY");
    crate::env::set_var("IAGENT_OPENROUTER_ENV_FILE", "alt-openrouter.env");
    crate::env::set_var("IAGENT_OPENAI_COMPAT_API_KEY_NAME", "ALT_COMPAT_KEY");
    crate::env::set_var("IAGENT_OPENAI_COMPAT_ENV_FILE", "alt-compat.env");
...
fn named_provider_profile_maps_to_openai_compatible_runtime_env() {
    ...
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_MODEL_CATALOG")
            .ok()
            .as_deref(),
        Some("0")
    );
    ...
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_AUTH_HEADER")
            .ok()
            .as_deref(),
        Some("api-key")
    );
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_AUTH_HEADER_NAME")
            .ok()
            .as_deref(),
        Some("x-api-key")
    );
    assert_eq!(
        std::env::var("IAGENT_NAMED_PROVIDER_PROFILE")
            .ok()
            .as_deref(),
        Some("my-gateway")
    );
...
fn named_provider_inline_api_key_is_private_runtime_fallback() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "IAGENT_OPENROUTER_MODEL_CATALOG",
        "IAGENT_NAMED_PROVIDER_PROFILE",
        "IAGENT_PROVIDER_MY_GATEWAY_API_KEY",
    ]);
    ...
    assert_eq!(
        std::env::var("IAGENT_OPENROUTER_API_KEY_NAME")
            .ok()
            .as_deref(),
        Some("IAGENT_PROVIDER_MY_GATEWAY_API_KEY")
    );
    assert_eq!(
        std::env::var("IAGENT_PROVIDER_MY_GATEWAY_API_KEY")
            .ok()
            .as_deref(),
        Some("inline-secret")
    );
...
fn matrix_openrouter_like_sources_reject_invalid_overrides() {
    ...
    crate::env::set_var("IAGENT_OPENROUTER_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("IAGENT_OPENROUTER_ENV_FILE", "../bad.env");
    crate::env::set_var("IAGENT_OPENAI_COMPAT_API_KEY_NAME", "bad key");
    crate::env::set_var("IAGENT_OPENAI_COMPAT_ENV_FILE", "../bad-compat.env");
...
fn matrix_openai_compatible_profile_overrides_read_from_env_file() {
    ...
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_API_BASE");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_DEFAULT_MODEL");
    std::fs::write(
        config_root.join(OPENAI_COMPAT_PROFILE.env_file),
        concat!(
            "IAGENT_OPENAI_COMPAT_API_BASE=https://api.example.com/v1\n",
            "IAGENT_OPENAI_COMPAT_API_KEY_NAME=EXAMPLE_API_KEY\n",
            "IAGENT_OPENAI_COMPAT_ENV_FILE=example.env\n",
            "IAGENT_OPENAI_COMPAT_DEFAULT_MODEL=example/model\n",
        ),
    )
    .expect("env file");
...
fn matrix_openai_compatible_localhost_override_allows_no_auth() {
    ...
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("IAGENT_OPENAI_COMPAT_LOCAL_ENABLED");
...
fn matrix_load_api_key_from_env_or_config_prefers_env() {
    ...
    let _guard = EnvGuard::save(&["IAGENT_HOME", "OPENCODE_API_KEY"]);
    crate::env::set_var("IAGENT_HOME", temp.path());
...
fn load_api_key_accepts_legacy_zai_key_name() {
    ...
    let _guard = EnvGuard::save(&["IAGENT_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
    crate::env::set_var("IAGENT_HOME", temp.path());

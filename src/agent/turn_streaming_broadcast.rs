...
impl Agent {
    pub(super) async fn run_turn_streaming(
        ...
        loop {
            ...
            let mut think_filter = crate::think_filter::ThinkTagFilter::default();
            ...
            loop {
                ...
                match event {
                    ...
                    StreamEvent::TextDelta(text) => {
                        let text = think_filter.push(&text);
                        if text.is_empty() {
                            continue;
                        }
            ...
            if self.provider.handles_tools_internally() {
                tool_calls.retain(|tc| IAGENT_NATIVE_TOOLS.contains(&tc.name.as_str()));
                if tool_calls.is_empty() {
            ...
            for tool_index in 0..tool_count {
                ...
                let is_native_tool = IAGENT_NATIVE_TOOLS.contains(&tc.name.as_str());

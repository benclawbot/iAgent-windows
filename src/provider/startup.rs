...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
            ...
            && let Some(pref) = provider_state.default_provider_key()
        {
                ...
                crate::provider_catalog::resolve_openai_compatible_profile_selection(pref)
            {
                ...
                match crate::provider_catalog::apply_named_provider_profile_env_from_config(
                    ...
                    Ok(profile_name) => {
                        crate::env::set_var("IAGENT_PROVIDER_PROFILE_NAME", &profile_name);
        ...
        let use_claude_cli = std::env::var("IAGENT_USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "IAGENT_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by IAGENT_USE_CLAUDE_CLI=1)",
            );
        ...
        let copilot_api = if has_copilot_api {
            ...
            match copilot::CopilotApiProvider::new() {
                ...
                Err(e) => {
                    log_info!(("Failed to initialize Copilot API: {}", e));
                    None
                }
            }
        } else {
        ...
        let openrouter = if has_openrouter_creds {
            let named_profile = std::env::var("IAGENT_NAMED_PROVIDER_PROFILE")
                .ok()
            ...
            match provider_result {
                Ok(p) => Some(Arc::new(p)),
                Err(e) => {
                    log_info!(("Failed to initialize OpenRouter: {}", e));
                    None
                }
            }
        } else {
    ...
    pub(super) fn auto_select_multi_account_for_provider(&self, provider: ActiveProvider) {
        ...
        if probe.all_accounts_exhausted() {
            log_info!(("All {} accounts are exhausted", provider_name));
            let notice = format!(

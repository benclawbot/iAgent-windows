...
#[cfg_attr(
    windows,
    ignore = "requires isolated full auth probes not available on Windows CI"
)]
#[test]
fn full_and_fast_auth_status_match_for_shared_probe_fields() {
    ...
    assert_eq!(full.iagent, AuthState::Available);
...
fn assert_auth_status_shared_fields_match(full: &AuthStatus, fast: &AuthStatus) {
    assert_eq!(full.iagent, fast.iagent, "iagent");
    assert_eq!(

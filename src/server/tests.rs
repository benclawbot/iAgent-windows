...
fn configure_test_env(root: &tempfile::TempDir) -> EnvGuard {
    ...
    crate::env::set_var("IAGENT_HOME", &home_dir);
...
#[allow(
    ...
    reason = "test intentionally serializes process-wide IAGENT_HOME/env state across async recovery assertions"
)]
...
#[allow(
    clippy::await_holding_lock,
    reason = "test intentionally serializes process-wide IAGENT_HOME/env state across async startup assertions"
)]
async fn startup_ready_signal_is_not_blocked_by_headless_recovery_delay() -> Result<()> {
    ...
    let temp = tempfile::TempDir::new()?;
    let _env = configure_test_env(&temp);
    let _delay_guard = ScopedEnvVar::set("IAGENT_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS", "500");
    ...
    let _ready_fd_guard = ScopedEnvVar::set("IAGENT_READY_FD", write_fd.to_string());

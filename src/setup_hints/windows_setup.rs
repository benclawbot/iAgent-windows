...
fn create_hotkey_shortcut(use_alacritty: bool) -> Result<()> {
    ...
    let _ = std::process::Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "Get-Process powershell, pwsh -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*iagent-hotkey*' } | Stop-Process -Force -ErrorAction SilentlyContinue",
        ])
        .output();

    let ps1_path = hotkey_dir.join("iagent-hotkey.ps1");
    let ps1_content = format!(
        r#"# iagent Alt+; global hotkey listener
# Auto-generated by iagent setup-hotkey. Runs at login via startup shortcut.
...
"#,
    ...
    let vbs_path = hotkey_dir.join("iagent-hotkey-launcher.vbs");
...
$shortcut = $shell.CreateShortcut("{startup_dir}\iagent-hotkey.lnk")
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\iagent.lnk", desktop_dir);
...
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            log_info!(("Created desktop shortcut: {}", shortcut_path));
        }
    }

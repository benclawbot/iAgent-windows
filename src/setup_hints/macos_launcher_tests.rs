...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/iagent",
        Path::new("/Users/test/Applications/Iagent.app"),
    );
    assert!(script.contains("display alert \"Iagent launch failed\""));
    assert!(script.contains("iagent setup-launcher"));
...
fn macos_launcher_refreshes_when_new_bundle_missing() {
    ...
    let app_dir = temp.path().join("Iagent.app");
    let legacy_app_dir = temp.path().join("iagent.app");
